...
def save_imgs(img_chunks, imgfolder, outputdir, filetype):
    ...
    for f in img_chunks[0]:
        shutil.copy(imgfolder + f, outputdir[0] + filetype)
        
    if not os.path.exists(outputdir[1] + filetype):
        os.makedirs(outputdir[1] + filetype)
    for f in img_chunks[1]:
        shutil.copy(imgfolder + f , outputdir[1] + filetype)
        
    if not os.path.exists(outputdir[2] + filetype):
        os.makedirs(outputdir[2] + filetype)
    for f in img_chunks[2]:
        shutil.copy(imgfolder + f, outputdir[2] + filetype)

if __name__== "__main__":
    parser = ArgumentParser(description="Split dataset into multiple chunks provided the ratio.\n"
                                        "Example: [python3 split.py --filetype [IMG, CLI, RNA]"
                                        " --ratio 10 10 80 --seed 13 --outputdir data_splits/]", formatter_class=RawTextHelpFormatter)
    parser.add_argument("-ftype","--filetype", dest = 'filetype',
                        required = True, help="input file type")
    ...
    parser.add_argument("-o","--outputdir", default='data_splits/', dest = 'outdir', help="output folder path")
    ...
    if filetype in ['CLI', 'RNA']:
        if filetype == 'CLI':
            filename = DATA_DIR + 'csv/cli_data_processed.csv'
        else:
            filename = DATA_DIR + 'csv/mRNAseq.csv'
        ...
        print('Known filetype passed, known filetypes are IMG, CLI, RNA')