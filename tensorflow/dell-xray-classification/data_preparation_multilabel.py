...
class DataPreparation:
    ...
    def preprocess_metadata(metadata, minimum_cases=params.MIN_CASES):
        ...
        print("Total x-ray records after removing images with "
              "No Finding Labels: {}.".format((metadata.shape[0])))
    ...
    def prepare_dataset_csv(self, train, valid):
        ...
        dataframe.to_csv(os.path.join(
            params.INPUT_DATA_DIR, params.TRAINING_CSV_FILE),
            columns=column_names,
            index=False, float_format=None)
        # Validation DataFrame
        dataframe = pd.DataFrame(valid)
        dataframe.to_csv(os.path.join(
            params.INPUT_DATA_DIR, params.VALID_CSV_FILE),
            columns=column_names,
            index=False, float_format=None)
    ...
    def create_dataset(self, train, valid):
        ...
        # create images dir
        if not os.path.exists(os.path.join(params.INPUT_DATA_DIR, 'images')):
            os.makedirs(os.path.join(params.INPUT_DATA_DIR, 'images'))
    ...
    def copy_images(self, dataframe):
        ...
        dst_dir = os.path.join(params.INPUT_DATA_DIR, 'images')
