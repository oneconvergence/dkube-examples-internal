...
This example is derived from [retraining](https://www.tensorflow.org/hub/tutorials/image_retraining) and modified to run on Dkube Platform. The example retrains a [resnetv2](https://tfhub.dev/google/imagenet/resnet_v2_50/feature_vector/1) model for categorical classification with cifar data.

 - This program trains RESNETV2 network on cifar data.
...
# Directories
 ...
 - **classifier/data**: This directory has processed training and validation data. Program trains on this data.
 ...
 - **pipeline/cifar.py**: Python DSL defining a sample Dkube pipeline. Pipeline uses Dkube components for all stages.
 - **pipeline/cifar.tar.gz**: Compiled python DSL which can be directly uploaded on to Dkube platform and executes using CPU.
 - **pipeline/cifar-gpu.tar.gz**: Compiled python DSL which can be directly uploaded on to Dkube platform and executes using GPU. 
 - **pipeline/cifar.ipynb**: Ipython notebook with the code. Upload the file in Dkube notebook and run all the cells. This notebook will generate and trigger the run of pipeline.
...
## Step1: Create a workspace
 ...
 4. Enter a unique name say *cifar-workspace*
 5. Paste link *[https://github.com/oneconvergence/dkube-examples/tree/cifar10/tensorflow/classification/cifar10/classifier/program](https://github.com/oneconvergence/dkube-examples/tree/cifar10/tensorflow/classification/cifar10/classifier/program)* in the URL text box.
...
## Step2: Create a dataset
 ...
 3. Select *Github* option.
 4. Enter a unique name say *cifar-dataset*
 5. Paste link *[https://github.com/oneconvergence/dkube-examples/tree/cifar10/tensorflow/classification/cifar10/classifier/data](https://github.com/oneconvergence/dkube-examples/tree/cifar10/tensorflow/classification/cifar10/classifier/data)* in the URL text box.
...
## Step3: Start a training job
 ...
 3. Fill the fields in Job form and click *Submit* button. Toggle *Expand All* button to auto expand the form. See below for sample values to be given in the form, for advanced usage please refer to **Dkube User Guide**.
	- Enter a unique name say *cifar-classifier*
	- **Container** section
	...
	- **Workspace** section - Please select the workspace *cifar-workspace* created in *Step1*.
	- **Model** section - Do not select any model.
	- **Dataset** section - Please select the dataset *cifar-dataset* created in *Step1*.
4. Click *Submit* button.
5. A new entry with name *cifar-classifier* will be created in *Jobs* table.
...
# How to Serve
 ...
 4. Input the unique name say *cifar-serving*
 5. Select *CPU* or *GPU* to deploy model on specific device. Unless specifically required, model can be served on CPU.
 6. Click *Deploy* button.
 7. Click *Inferences* side menu and check that a serving job is created with the name given i.e, *cifar-serving*.
...
3. Create a file *cifar.ini* with below contents, Only field to be filled in is *modelurl*. Paste the *URL* copied in previous step.
    ...
    #kubeconfig=""
    ```
  4. Execute the command `dkubectl infapp launch --config cifar.ini -n cifar`
...
## How to use cifar.ipynb
1. Create a new *Workspace* in Dkube as explained in [section](##%20Step1:%20Create%20a%20workspace) with unqiue name say, *cifar-pl-nb*
2. Change the github url to [https://github.com/oneconvergence/dkube-examples/blob/cifar10/tensorflow/classification/cifar10/pipeline/cifar.ipynb](https://github.com/oneconvergence/dkube-examples/blob/cifar10/tensorflow/classification/cifar10/pipeline/cifar.ipynb)
3. Create a new *Notebook* in Dkube and select the workspace as *cifar-pl-nb*.
4. The Dkube notebook does not need any Dataset.
5. Launch the notebook and wait for the status to show *running*.
6. Then, click the *Jupyter* icon which will open a UI. Selec the *cifar.ipynb* and double click it to open.
7. Run all the cells of *cifar.ipynb*. This will create a pipeline, creates an experiment and a run.
8. Links are displayed in the output cells wherever applicable.
## How to use cifar.tar.gz
